...
import com.shatteredpixel.shatteredpixeldungeon.sprites.DarkKingSprite;
...
public class DarkKing extends Mob {

    // WIP and will be used in a later version if we ever get to that point
    // currently is unchanged and just a reskin of the dwarf king

	{
		spriteClass = DarkKingSprite.class;
			...
			if (delay <= 0){
				...
				if (Actor.findChar(pos) == null) {
					...
					if (!ch.isAlive() && ch == Dungeon.hero) {
						Dungeon.fail(DarkKing.class);
						GLog.n( Messages.capitalize(Messages.get(Char.class, "kill", Messages.get(DarkKing.class, "name"))));
					}
				}
