...
import com.shatteredpixel.shatteredpixeldungeon.sprites.FrozenSwarmSprite;
...
public class FrozenSwarm extends Mob {
    ...
    public int defenseProc( Char enemy, int damage ) {
        ...
        if (HP >= damage + 2) {
            ...
            // Calculate splits based on HP AFTER taking damage
            int remainingHP = HP - damage;

            for (int splitCount = 0; splitCount < Random.Int(1, 3); splitCount++) {
                ...
                if (candidates.size() > 0 && remainingHP >= 3) {

                    FrozenSwarm clone = split();
            ...
            // Apply the final HP after all splits
            HP = remainingHP;
        }
	...
	private FrozenSwarm split() {
		FrozenSwarm clone = new FrozenSwarm();
