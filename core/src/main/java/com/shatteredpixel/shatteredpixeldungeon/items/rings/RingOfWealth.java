...
public class RingOfWealth extends Ring {
    ...
    public String statsInfo() {
        if (isIdentified()){
            String info = Messages.get(this, "stats",
                    Messages.decimalFormat("#.##", 100f * (Math.pow(1.075f, soloBuffedBonus()) - 1f)));
            if (isEquipped(Dungeon.hero) && soloBuffedBonus() != combinedBuffedBonus(Dungeon.hero)){
                info += "\n\n" + Messages.get(this, "combined_stats",
                        Messages.decimalFormat("#.##", 100f * (Math.pow(1.075f, combinedBuffedBonus(Dungeon.hero)) - 1f)));
            }
            return info;
        } else {
            return Messages.get(this, "typical_stats", Messages.decimalFormat("#.##", 7.5f));
        }
    }

    public String upgradeStat1(int level){
        if (cursed && cursedKnown) level = Math.min(-1, level-3);
        return Messages.decimalFormat("#.##", 100f * (Math.pow(1.075f, level+1)-1f)) + "%";
    }
    ...
    public static float dropChanceMultiplier( Char target ){
        return (float)Math.pow(1.075, getBuffedBonus(target, Wealth.class));
    }
    ...
    public static ArrayList<Item> tryForBonusDrop(Char target, int tries ){
        ...
        if (triesToDrop == null){
            ...
            triesToDrop.countUp( Random.NormalIntRange(10, 32) );
        }
        ...
        if (dropsToEquip == null){
            ...
            dropsToEquip.countUp( Random.NormalIntRange(12, 25) );
        }
    ...
    public static Item genConsumableDrop(int level) {
        ...
        if (roll < (0.75f - 0.03f * level)) {
            latestDropTier = 1;
            return genLowValueConsumable();

        } else if (roll < (0.9f - 0.01f * level)) {
            latestDropTier = 2;
    ...
    private static Item genEquipmentDrop( int level ){
        ...
        int floorset = (Dungeon.depth + (int)(level/2)/5);
        switch (Random.Int(5)){
