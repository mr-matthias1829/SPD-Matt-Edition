...
public class IcyCoreArmor extends ArmorAugment {

    private static final float CHILL_CHANCE = 0.06f;
    ...
    public int proc(Armor armor, Char attacker, Char defender, int damage) {
        if (attacker != null && Dungeon.level.adjacent(attacker.pos, defender.pos)) {
            if (Random.Float() < CHILL_CHANCE) {
                int add = armor.level() / 5; // every 5 levels on a armor adds 1 turn to chilling
                Buff.affect(attacker, Chill.class, CHILL_DURATION+add);
            }
        }
        return damage;
    }
