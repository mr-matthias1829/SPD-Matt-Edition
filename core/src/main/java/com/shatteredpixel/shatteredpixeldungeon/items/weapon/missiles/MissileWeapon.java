...
abstract public class MissileWeapon extends Weapon {
    ...
    public int STRReq(int lvl){
        int req = STRReq(tier, lvl, getClass()) - 1; //1 less str than normal for their tier
    ...
    public Item merge(Item other) {
        ...
        if (isSimilar(other)) {
            ...
            // merge augments: keep the augment from whichever stack has the more significant one.
            // prefer any non-trivial augment; if both are non-trivial, keep ours (already set).
            if (augment instanceof NoWeaponAugment && !(((MissileWeapon) other).augment instanceof NoWeaponAugment)){
                augment = ((MissileWeapon) other).augment;
            }
        }
        return this;
    }
    ...
    public String info() {
        ...
        // Display augment description when a non-trivial augment is present.
        // Uses the augment's own name/desc rather than hard-coded strings,
        // so any WeaponAugment subclass is handled automatically.
        if (!(augment instanceof NoWeaponAugment)) {
            info += " " + augment.name();
            String augDesc = augment.desc();
            if (augDesc != null && !augDesc.isEmpty()) {
                info += ": " + augDesc;
            }
        }
