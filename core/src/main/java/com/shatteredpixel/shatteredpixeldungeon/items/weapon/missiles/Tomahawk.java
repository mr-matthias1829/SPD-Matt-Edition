...
public class Tomahawk extends MissileWeapon {
    ...
    public int proc( Char attacker, Char defender, int damage ) {
        ...
        float bleed = Random.NormalFloat(minBleed(), maxBleed());
        Buff.affect( defender, Bleeding.class ).set( augment.damageFactor(Math.round(bleed)) );
    ...
    public String statsInfo(){
        if (isIdentified()){
            return Messages.get(this, "stats_desc",
                    Math.round(augment.damageFactor(Math.round(minBleed()))),
                    Math.round(augment.damageFactor(Math.round(maxBleed()))));
        } else {
            return Messages.get(this, "typical_stats_desc",
                    Math.round(augment.damageFactor(Math.round(minBleed(0)))),
                    Math.round(augment.damageFactor(Math.round(maxBleed(0)))));
        }
    }
