...
public class ItemSlot extends Button {
    ...
    protected BitmapText magicLevelText; // NEW: shows magic level for armor
    ...
    protected void createChildren() {
        ...
        // magic level text (blue by default)
        magicLevelText = new BitmapText( PixelScene.pixelFont);
        add(magicLevelText);
    }
    ...
    protected void layout() {
        ...
        if (itemIcon != null){
            ...
            if (magicLevelText != null && item instanceof Armor) {
                Armor a = (Armor) item;
                magicLevelText.measure();

                // Always position relative to the bottom-right corner, not relative to level text
                // Place it above where the normal level would be
                float levelHeight = level != null && level.text().length() > 0 ? level.height() + 2 : 0;
                magicLevelText.x = x + (width - magicLevelText.width()) - margin.right;
                magicLevelText.y = y + (height - magicLevelText.baseLine() - 1 - levelHeight) - margin.bottom;
                PixelScene.align(magicLevelText);
            }
    }
    ...
    public void alpha( float value ){
        ...
        if (magicLevelText != null) magicLevelText.alpha(value);
    }
    ...
    public void updateText(){
        ...
        if (item == null){
            status.visible = extra.visible = level.visible = magicLevelText.visible = false;
            return;
        } else {
            status.visible = extra.visible = level.visible = magicLevelText.visible = true;
        }
        ...
        // NEW: show armor magic level in a separate small label (blue)
        magicLevelText.visible = false; // default to hidden
        if (item instanceof Armor) {
            Armor a = (Armor)item;
            if (a.magicLevel > 0) {
                magicLevelText.text( Messages.format( TXT_LEVEL, a.magicLevel ) );
                magicLevelText.measure();
                magicLevelText.hardlight( 0x4488FF ); // nice blue color
                magicLevelText.visible = true;
            }
        }
    ...
    public void enable( boolean value ) {
        ...
        if (magicLevelText != null) magicLevelText.alpha( alpha );
    }
