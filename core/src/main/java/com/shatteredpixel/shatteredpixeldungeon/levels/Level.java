...
public abstract class Level implements Bundlable {
    ...
    public int splashRegionOverride = -1; // -1 means use default region calc
    ...
    public boolean canSpawnFood = true;       // bool for rations each floor, very essential to have separated
    public boolean canFeeling = true;         // bool for whether this level can have a feeling

    /**
     * When > 0, overrides the normal random item count calculation entirely.
     * The level will spawn exactly this many random items (before feeling bonuses).
     * Does not affect special drops (petals, guide pages, guaranteed items, etc.).
     * Set per-category counts below to control what types of items fill those slots;
     * any unassigned slots are filled with fully random items as normal.
     */
    public int setItemsToSpawn = -1;

    /**
     * When setItemsToSpawn > 0, how many of those slots should be X drops.
     * -1 means no override for this category (slots are filled randomly).
     * this can be quite tricky and finicky to manage, so take care
     * variables above another variable takes priority over that variable
     */
    public int setScrollPotionCount = -1;
    public int setWeaponArmorCount  = -1;
    public int setMissileCount      = -1;
    public int setWandCount         = -1;
    public int setRingCount         = -1;
    public int setArtifactCount     = -1;
        ...
        if (!Dungeon.bossLevel() && Dungeon.branch != 1 && canSpawnItems) {
            ...
            if (Dungeon.depth > 1) {
                ...
                if (canFeeling) {
                    ...
                    feeling = Feeling.NONE; // no feeling if can't get one
                }
            }
        }
    ...
    public float respawnCooldown(){
        ...
        if (Statistics.amuletObtained){
            if (Dungeon.depth == 1){
                ...
                cooldown = (Dungeon.level.mobCount()) * (TIME_TO_RESPAWN / 40f);
            } else {
                //respawn time is 5/5/10/15/20/25/25, etc.
                cooldown = Math.round(GameMath.gate( TIME_TO_RESPAWN/5f, Dungeon.level.mobCount() * (TIME_TO_RESPAWN / 5f), TIME_TO_RESPAWN / 2f));
            }
